...
namespace RainbowBlockSaga.Presentation.Scripts.GUI.Labels
{
    public class LabelAnim : MonoBehaviour
    {
        ...
        // custom for shop
        public void AnimateFree(Vector3 startPos, Vector3 endPos,Vector3 popupTextPos,string rewardDataCount, AudioClip sound, Action callback)
        {
            int completeCount = 0;
            int animateCount = 4;
            if (coinsTextPrefab != null)
            {
                PopupText(popupTextPos, rewardDataCount);
            }
            for (int i = 0; i < animateCount; i++)
            {
                // clone icon bằng pool
                var item = PoolObject.GetObject(icon.gameObject, startPos);

                // RESET SCALE để tránh icon phóng to
                item.transform.localScale = Vector3.one;

                // random start
                float r = 0.5f;
                item.transform.position = startPos +
                    new Vector3(Random.Range(-r, r), Random.Range(-r, r));

                StartAnim(item.transform, endPos, () =>
                {
                    // punch scale tại startPos
                    var punch = transform.DOPunchScale(Vector3.one * 0.2f, 0.2f);
                    punch.OnComplete(() =>
                    {
                        // RESET SCALE LABEL TRÁNH PHÓNG TO
                        transform.localScale = Vector3.one;
                    });
                    ...
                    if (fxPrefab != null)
                    {
                        var fx = PoolObject.GetObject(fxPrefab, endPos);
                        fx.transform.localScale = Vector3.one;
                        DOVirtual.DelayedCall(1f, () => PoolObject.Return(fx));
                    }

                    if (completeCount == 0 && sound != null)
                        SoundBase.instance.PlaySound(sound);

                    completeCount++;
                    if (completeCount == animateCount)
                        callback?.Invoke();

                    // RESET SCALE TRƯỚC KHI RETURN POOL
                    item.transform.localScale = Vector3.one;
