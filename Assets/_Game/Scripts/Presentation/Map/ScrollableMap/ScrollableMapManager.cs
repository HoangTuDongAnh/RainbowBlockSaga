...
namespace RainbowBlockSaga.Presentation.Scripts.Map.ScrollableMap
{
    public class ScrollableMapManager : SingletonBehaviour<ScrollableMapManager>
    {
        ...
        public bool unlockAllLevelsInTest = false;
        ...
        private void DuplicateDecorations(Transform[] originalDecorations, float verticalOffset)
        {
            foreach (Transform original in originalDecorations)
            {
                ...
                GameObject prefabToUse = original.gameObject;
                
                // Randomly replace with alternative decoration if available
                if (randomDecorationPrefabs != null && randomDecorationPrefabs.Length > 0 && Random.value < replacementChance)
                {
                    prefabToUse = randomDecorationPrefabs[Random.Range(0, randomDecorationPrefabs.Length)];
                }
                
                var copy = Instantiate(prefabToUse, newPosition, original.rotation, levelsGrid);
